...
static uint32_t dayOfYear(uint16_t year, uint8_t month, uint8_t day) {
...
static bool calculateSunsetAndSunrise(float gamma, float latitude, float longitude, uint16_t *sunriseMinutes, uint16_t *sunsetMinutes) {
    ...
    *sunriseMinutes = (uint16_t)roundf(sunrise);

    *sunsetMinutes = (uint16_t)roundf(sunset);
...
bool Sunrise_calculateFromUnix(uint32_t currentTime, float latitude, float longitude, uint16_t *sunriseMinutes, uint16_t *sunsetMinutes) {
    ...
    uint16_t year = 1900 + time->tm_year;
